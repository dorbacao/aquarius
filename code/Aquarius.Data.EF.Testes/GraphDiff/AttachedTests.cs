...
namespace Aquarius.Data.EF.Testes.GraphDiff
{
    ...
    public class AttachedTests
    {
        ...
        [TestMethod]
        public void Attached_OwnedCollectionWithAssociatedCollectionShouldNotUpdate()
        {
            Models.Company company1;
            ...
            using (var context = new TestDbContext(_connection))
            {
                ...
                var originalEmail = company1.Contacts.First().Infos.First().Email;
                company1.Contacts.First().Infos.First().Email = "alterado";
                Assert.AreNotEqual(originalEmail, company1.Contacts.First().Infos.First().Email);

                company1.Contacts.First()
                    .Infos.Add(new Models.ContactInfo { Description = "Test", Email = "test@test.com" });

                // Setup mapping
                context.UpdateGraph(company1, map => map
                    .OwnedCollection(p => p.Contacts, with => with
                        .AssociatedCollection(m => m.Infos)));
            ...
            using (var context = new TestDbContext(_connection))
            {
                var value = context.Companies.Include(p => p.Contacts.Select(m => m.Infos)).First();

                Assert.AreEqual(2, value.Contacts.First().Infos.Count);
                Assert.AreNotEqual("alterado", value.Contacts.First().Infos.First().Email);
            }
        }
